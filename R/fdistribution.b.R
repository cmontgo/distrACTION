FDistributionClass <- if (requireNamespace('jmvcore')) R6::R6Class(
    ...
    private = list(
      ...
      .run = function() {
        ...
        # The specification of the second distribution parameter (df2) is extracted
        DP2 <- self$options$dp2
        # The specification of the third distribution parameter (df2) is extracted
        ...
        if(DP1<5){
          UpperTail <- 15}
        if(((UpperTail-LowerTail)/2)>((qf(.9999, DP1, DP2, DP3))-LowerTail)){
          UpperTail <- ceiling(qf(.9995, DP1, DP2, DP3)*2)/2
          SpecialWidth <- "TRUE"}
        ...
        InputSummary <- matrix(ncol = 3, nrow = 3)
        ...
        Inputs$setRow(rowNo=3, values=list(
          ParametersColumn=InputSummary[3,1],
          DistributionFunctionColumn=InputSummary[3,2],
          QuantileFunctionColumn=InputSummary[3,3]))
        ...
        Density <- df(x, DP1, DP2, DP3)
        if(DistributionFunction=="TRUE"){
          # Calculation of the quantile
          DistributionResult1 <- pf(XValue, DP1, DP2, DP3)
          ...
          if(DistributionFunctionType == "interval"){
            ...
            DistributionResult2 <- pf(XValue2, DP1, DP2, DP3)
        ...
        if(max(Datas$Prob)=="Inf"){
          Datas[(Datas[,2]==max(Datas$Prob)),] <- c(0,5, df(0.5, DP1, DP2, DP3))}
        ...
        if(SpecialWidth=="TRUE"){
          ...
          if(length(AxisSegments)<5)
            AxisSegments <- seq(LowerAxisSegment, ceiling(HigherAxisSegment), by = 0.25)}
        ...
        if(QuantileFunction=="TRUE"){
          ...
          HigherSegmentLength <- df(HigherSegment, DP1, DP2, DP3)
