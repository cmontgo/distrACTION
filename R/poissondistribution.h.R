
# This file is automatically generated, you probably don't want to edit this

PoissonDistributionOptions <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "PoissonDistributionOptions",
    inherit = jmvcore::Options,
    public = list(
        initialize = function(
            DistributionFunction = FALSE,
            QuantileFunction = FALSE,
            DistributionFunctionType = "is",
            x1 = 0,
            p = 0.5,
            x2 = 1,
            dp1 = 10, ...) {

            super$initialize(
                package="distrACTION",
                name="PoissonDistribution",
                requiresData=FALSE,
                ...)

            private$..DistributionFunction <- jmvcore::OptionBool$new(
                "DistributionFunction",
                DistributionFunction,
                default=FALSE)
            private$..QuantileFunction <- jmvcore::OptionBool$new(
                "QuantileFunction",
                QuantileFunction,
                default=FALSE)
            private$..DistributionFunctionType <- jmvcore::OptionList$new(
                "DistributionFunctionType",
                DistributionFunctionType,
                options=list(
                    "lower",
                    "higher",
                    "interval",
                    "is"),
                default="is")
            private$..x1 <- jmvcore::OptionNumber$new(
                "x1",
                x1,
                default=0)
            private$..p <- jmvcore::OptionNumber$new(
                "p",
                p,
                min=0,
                max=1,
                default=0.5)
            private$..x2 <- jmvcore::OptionNumber$new(
                "x2",
                x2,
                default=1)
            private$..dp1 <- jmvcore::OptionNumber$new(
                "dp1",
                dp1,
                default=10)

            self$.addOption(private$..DistributionFunction)
            self$.addOption(private$..QuantileFunction)
            self$.addOption(private$..DistributionFunctionType)
            self$.addOption(private$..x1)
            self$.addOption(private$..p)
            self$.addOption(private$..x2)
            self$.addOption(private$..dp1)
        }),
    active = list(
        DistributionFunction = function() private$..DistributionFunction$value,
        QuantileFunction = function() private$..QuantileFunction$value,
        DistributionFunctionType = function() private$..DistributionFunctionType$value,
        x1 = function() private$..x1$value,
        p = function() private$..p$value,
        x2 = function() private$..x2$value,
        dp1 = function() private$..dp1$value),
    private = list(
        ..DistributionFunction = NA,
        ..QuantileFunction = NA,
        ..DistributionFunctionType = NA,
        ..x1 = NA,
        ..p = NA,
        ..x2 = NA,
        ..dp1 = NA)
)

PoissonDistributionResults <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "PoissonDistributionResults",
    inherit = jmvcore::Group,
    active = list(
        Inputs = function() private$.items[["Inputs"]],
        Outputs = function() private$.items[["Outputs"]],
        plot = function() private$.items[["plot"]]),
    private = list(),
    public=list(
        initialize=function(options) {
            super$initialize(
                options=options,
                name="",
                title="Poisson Distribution")
            self$add(jmvcore::Table$new(
                options=options,
                name="Inputs",
                title="Input values",
                rows=2,
                columns=list(
                    list(
                        `name`="ParametersColumn", 
                        `title`="Parameters", 
                        `type`="text"),
                    list(
                        `name`="DistributionFunctionColumn", 
                        `title`="'Compute probability'", 
                        `type`="text", 
                        `visible`="(DistributionFunction)"),
                    list(
                        `name`="QuantileFunctionColumn", 
                        `title`="'Compute quantile(s)'", 
                        `type`="text", 
                        `visible`="(QuantileFunction)"))))
            self$add(jmvcore::Table$new(
                options=options,
                name="Outputs",
                title="Results",
                rows=1,
                clearWith=list(
                    "group",
                    "alt",
                    "varEq"),
                columns=list(
                    list(
                        `name`="DistributionResultColumn", 
                        `title`="Probability", 
                        `type`="number", 
                        `format`="zto", 
                        `visible`="(DistributionFunction)"),
                    list(
                        `name`="QuantileResultColumn",
                        `title`="x1",
                        `type`="integer",
                        `visible`="(QuantileFunction)",
                        `superTitle`="Quantile"),
                    list(
                        `name`="MeanColumn",
                        `title`="Mean",
                        `type`="text",
                        `superTitle`="Distribution Statistics"),
                    list(
                        `name`="SDColumn",
                        `title`="SD",
                        `type`="text",
                        `superTitle`="Distribution Statistics"))))
            self$add(jmvcore::Image$new(
                options=options,
                name="plot",
                title="",
                width=500,
                height=400,
                renderFun=".plot"))}))

PoissonDistributionBase <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "PoissonDistributionBase",
    inherit = jmvcore::Analysis,
    public = list(
        initialize = function(options, data=NULL, datasetId="", analysisId="", revision=0) {
            super$initialize(
                package = "distrACTION",
                name = "PoissonDistribution",
                version = c(1,0,0),
                options = options,
                results = PoissonDistributionResults$new(options=options),
                data = data,
                datasetId = datasetId,
                analysisId = analysisId,
                revision = revision,
                pause = NULL,
                completeWhenFilled = FALSE,
                requiresMissings = FALSE,
                weightsSupport = 'na')
        }))

#' Poisson Distribution
#'
#' 
#' @param DistributionFunction .
#' @param QuantileFunction .
#' @param DistributionFunctionType .
#' @param x1 .
#' @param p .
#' @param x2 .
#' @param dp1 .
#' @return A results object containing:
#' \tabular{llllll}{
#'   \code{results$Inputs} \tab \tab \tab \tab \tab a table \cr
#'   \code{results$Outputs} \tab \tab \tab \tab \tab a table \cr
#'   \code{results$plot} \tab \tab \tab \tab \tab an image \cr
#' }
#'
#' Tables can be converted to data frames with \code{asDF} or \code{\link{as.data.frame}}. For example:
#'
#' \code{results$Inputs$asDF}
#'
#' \code{as.data.frame(results$Inputs)}
#'
#' @export
PoissonDistribution <- function(
    DistributionFunction = FALSE,
    QuantileFunction = FALSE,
    DistributionFunctionType = "is",
    x1 = 0,
    p = 0.5,
    x2 = 1,
    dp1 = 10) {

    if ( ! requireNamespace("jmvcore", quietly=TRUE))
        stop("PoissonDistribution requires jmvcore to be installed (restart may be required)")


    options <- PoissonDistributionOptions$new(
        DistributionFunction = DistributionFunction,
        QuantileFunction = QuantileFunction,
        DistributionFunctionType = DistributionFunctionType,
        x1 = x1,
        p = p,
        x2 = x2,
        dp1 = dp1)

    analysis <- PoissonDistributionClass$new(
        options = options,
        data = data)

    analysis$run()

    analysis$results
}

